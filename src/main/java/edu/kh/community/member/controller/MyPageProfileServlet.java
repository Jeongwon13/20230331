...
public class MyPageProfileServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// -> cos.jar에서 제공하는 FileRenamePolicy 인터페이스 상속 클래스를 생성.
		...
		// 2) 업로드 된 프로필 이미지의 웹 접근 경로(folderPath + 변경된 파일명)
		...
		// 2) delete의 값이 1이면(눌러진 경우) profileImage 값을 null로 변경
		if(delete == 1) {
			profileImage = null;
		} 
		...
		if(result > 0) { // 성공
			...
			loginMember.setProfilemage(profileImage); // 세션의 값이 변경됨.
			
		} else { // 실패
			session.setAttribute("message", "프로필 이미지 변경 실패ㅋ");
		}
